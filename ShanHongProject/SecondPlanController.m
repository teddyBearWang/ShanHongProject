...
#import "ThirdPlanController.h"

@interface SecondPlanController ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation SecondPlanController
...
{
    NSDictionary *select_dic = _listData[indexPath.row];
    if ([[select_dic objectForKey:@"PersonCD"] hasPrefix:@"s"]) {
        //PersonCD中含有S，表示县级本级
        PlanDetailController *detail = [[PlanDetailController alloc] init];
        detail.dic = select_dic;
        [self.navigationController pushViewController:detail animated:YES];
    }else{
        //push进入下一级
        ThirdPlanController *second = [[ThirdPlanController alloc] init];
        second.dic = select_dic;
